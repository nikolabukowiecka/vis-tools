...
if __name__ == "__main__":
  ...
  for fname in files:
    ...
    #vmin = 0
    #vmax = 6e-4
    im1 = plt.imshow(I, vmin=vmin, vmax=vmax , cmap='afmhot',  origin='lower', extent=extent)
    
    #for n = -1
    #vmax = np.log10(I.max())
    #vmin = (np.log10(I.max())-3)
    #im1 = plt.imshow(np.log10(I), vmin=vmin, vmax=vmax, cmap='afmhot',  origin='lower', extent=extent)
    
    #norm = matplotlib.colors.LogNorm(I.max()/1e3,I.max())
    #im1 = plt.imshow(I, cmap='afmhot',  origin='lower', extent=extent, norm = norm)

    plt.title("Stokes I [Jy ${\mu}as^{-2}$]")
    plt.ylabel("${\mu}as$")
    plt.xlabel("${\mu}as$")
    ...
    skip = int(npix/32) 
    plt.quiver(Xs[::skip,::skip],Ys[::skip,::skip],vxp[::skip,::skip],vyp[::skip,::skip], 
